...
```sh
git clone https://github.com/devp0nt/error0.git
cd error0
bun install
```
...
fix: handle undefined cause in deserialize
docs: clarify status plugin example
```

Mark a breaking change with `!`:

```
feat!: rename Error0.use("prop") to Error0.use("define")
```
